...
app.get('/public/home', async (c) => {
  ...
  const tagType = c.req.query('tagType'); // '番组' 或 '艺人' 或 null
  const tagId = c.req.query('tagId');     // 具体某个标签ID
  ...
  let tags = [];
  if(tagType) {
      tags = (await c.env.DB.prepare('SELECT * FROM tags WHERE type = ?').bind(tagType).all()).results;
  }
  ...
  if (tagType) { conditions.push('t.type = ?'); params.push(tagType); }
  if (tagId) { conditions.push('t.id = ?'); params.push(tagId); }
  ...
  if(resourceIds.length > 0) {
      ...
      const tagsData = await c.env.DB.prepare(`
        SELECT rt.resource_id, t.name, t.type, t.image_url 
  ...
  return c.json({ categories: categories.results, resources: safeResources, tags });
});
...
app.post('/resource/unlock', async (c) => {
  ...
  const { resourceId } = await c.req.json(); // 不再需要 blockIndex
...
app.post('/admin/users/batch', async (c) => {
    ...
    if(action==='mute') await c.env.DB.prepare(`UPDATE users SET is_muted=1 WHERE id IN (${ph})`).bind(...userIds).run();
    else if(action==='unmute') await c.env.DB.prepare(`UPDATE users SET is_muted=0 WHERE id IN (${ph})`).bind(...userIds).run();
    else if(action==='delete') await c.env.DB.prepare(`DELETE FROM users WHERE id IN (${ph})`).bind(...userIds).run();
    else if(action==='ban') {
        const users = await c.env.DB.prepare(`SELECT email FROM users WHERE id IN (${ph})`).bind(...userIds).all();
        for(const us of users.results) await c.env.DB.prepare('INSERT OR IGNORE INTO blacklist (email, reason) VALUES (?, "批量拉黑")').bind(us.email).run();
        await c.env.DB.prepare(`DELETE FROM users WHERE id IN (${ph})`).bind(...userIds).run();
    }
    return c.json({success:true});
});
...
app.post('/admin/resource', async (c) => {
  const t = c.req.header('Authorization')?.split(' ')[1]; const u = await verifyToken(t, c.env.JWT_SECRET); if(!u||u.role!=='admin') return c.json({error:'无权'},403);
  const { id, title, category_id, blocks, manualDate, tags } = await c.req.json(); // tags: [{name, type, image_url}]
  ...
  if (tags && tags.length > 0) {
      for (const tag of tags) {
          ...
          if (existing) {
              tagId = existing.id;
              // 如果新传了图片且旧的没有，或者想更新，这里简单处理：有新图就更新
              if (tag.image_url) {
                  await c.env.DB.prepare('UPDATE tags SET image_url = ? WHERE id = ?').bind(tag.image_url, tagId).run();
              }
          } else {
...
app.get('/admin/resource/:id', async (c) => {
  ...
  // 获取标签
  const tags = await c.env.DB.prepare(`SELECT t.name, t.type, t.image_url FROM resource_tags rt JOIN tags t ON rt.tag_id = t.id WHERE rt.resource_id = ?`).bind(r.id).all();
  return c.json({ ...r, blocks: JSON.parse(r.content_json), tags: tags.results });
});

// 其他管理员API保持不变...
app.get('/admin/resources', async (c) => { const t=c.req.header('Authorization')?.split(' ')[1]; const u=await verifyToken(t,c.env.JWT_SECRET); if(!u||u.role!=='admin') return c.json({error:'无权'},403); const q=c.req.query('q'); let sql='SELECT * FROM resources'; if(q) sql+=` WHERE title LIKE '%${q}%'`; sql+=' ORDER BY id DESC LIMIT 50'; const r=await c.env.DB.prepare(sql).all(); return c.json(r.results); });
app.post('/admin/resource/delete', async (c) => { const t=c.req.header('Authorization')?.split(' ')[1]; const u=await verifyToken(t,c.env.JWT_SECRET); if(!u||u.role!=='admin') return c.json({error:'无权'},403); await c.env.DB.prepare('DELETE FROM resources WHERE id=?').bind((await c.req.json()).id).run(); return c.json({success:true}); });
